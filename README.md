...
Powered by **Google's Gemini AI** and a lightweight **TF-IDF retrieval system**, our interactive chatbot offers highly contextual, safety-filtered advice regarding childcare, nutrition, and early disease symptoms without the heavy memory footprint of databases like ChromaDB.
...
├── requirements.txt            # Python dependencies (Optimized for Vercel <250 MB)
├── .env                        # Local environment variables
├── gemini_api_key.txt          # Fallback secret file for Gemini AI key
├── frontend/                   # HTML, CSS, JS frontend assets
│   ├── index.html              # Dashboard UI
│   ├── style.css               # Premium Glassmorphism styling
│   └── app.js                  # Asynchronous API handlers
├── chatbot/                    # Gemini AI & TF-IDF retriever
├── behaviour_analysis/         # Behavioral analysis model & notes
├── child_mortality/            # Health risk model & notes
├── celiac-disease/             # Celiac model & notes
...
Create a `.env` file in the root directory and paste your key (recommended):
```env
GEMINI_API_KEY=your_gemini_api_key_here
```
*(Alternatively, you can set the `GEMINI_API_KEY` or `GOOGLE_API_KEY` environment variable directly in your OS/hosting provider (e.g. Vercel), create a fallback `gemini_api_key.txt` file containing only the key, or save it directly through the UI dashboard settings).*
...
*(Alternatively, you can run it manually using Python:)*
```bash
python app.py
```
...
* **Machine Learning:** Scikit-Learn, Numpy (Optimized to run without Pandas, LightGBM, or XGBoost on the server to keep footprint under Vercel's limit!)
* **AI & NLP:** Google Generative AI (Gemini), Scikit-learn TF-IDF Vectorizer
